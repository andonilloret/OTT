...
(function() {
  var CEL, RedisStore, api, app, authRoutes, bodyParser, express, passport, passportPolicies, path, redis, server, session;
    ...
    if (req.session.user) {
      ...
      return api.getSchedule(function(error, response) {
        ...
        if (!error) {
          ...
          return res.render('pages/login', params);
        } else {
  ...
  app.get('/live', function(req, res) {
    ...
    return api.getSchedule(function(error, response) {
      if (!error) {
        params.schedule = JSON.parse(response.body);
        return res.render('pages/index', params);
      } else {
        return res.status(500).send('Ha habido un error cargando la página, por favor cargue de nuevo');
      }
    });
